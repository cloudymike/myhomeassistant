...
class HAIntegerSensor(HABaseDeviceType):
    def __init__(self, unique_id) -> None:
        super().__init__(constants.HAComponentIntegerSensor, unique_id)
        self._class = None
        self._icon = None
        self._expire_after = 0
        self._current_state = 0

    def set_state(self, state: int, force=False) -> bool:
    ...
    def build_serializer(self):
        ...
        self._serializer.set_topic(constants.HAAuxStateTopic)

    def on_mqtt_connected(self):
        if self.unique_id is None:
            return

        print("MHA: HAIntegerSensor on_mqtt_connected")
    ...
    def _publish_state(self, state) -> bool:
        return self.publish_on_data_topic(constants.HAAuxStateTopic, str(state))
